...
export default function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-[var(--ink)]/80 border-b border-[var(--border-on-ink)]">
      <div className="max-w-5xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
        <Link
          href="/"
          className="font-bold text-lg text-[var(--text-on-ink)] tracking-tight"
        >
          مبین کرم
        </Link>
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          <Link
            href="/"
            className="px-3 py-2 text-sm text-[var(--muted-on-ink)] hover:text-[var(--text-on-ink)] transition-colors"
          >
            خانه
          </Link>
          <Link
          ...
          <div className="w-px h-5 bg-[var(--border-on-ink)] mx-1 sm:mx-2" />
          <a
            href="https://github.com/Mobin-Karam"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="گیت‌هاب"
            className="p-2 text-[var(--muted-on-ink)] hover:text-[var(--brass-bright)] transition-colors"
          >
            <svg
              width="18"
              height="18"
